...
import { TrendingUp, ShoppingCart, ChevronLeft, ChevronRight } from "lucide-react"
import { fetchSalesReport, fetchPurchaseReport } from "../services/reportsService"
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex items-center gap-3 mb-6">
        <button onClick={() => handleMonthChange(-1)} className="p-1.5 border border-[#E8E4D9] rounded-md hover:bg-gray-50">
          <ChevronLeft size={16} />
        </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        ...
        >
          <TrendingUp size={20} className={activeReport === "sales" ? "text-white mb-2" : "text-[#0F4C3A] mb-2"} />
          <p className={`text-sm font-medium ${activeReport === "sales" ? "text-white" : "text-gray-900"}`}>
            Sales Report
          </p>
          <p className={`text-xs mt-1 ${activeReport === "sales" ? "text-green-200" : "text-[#8A8678]"}`}>
        ...
        >
          <ShoppingCart size={20} className={activeReport === "purchase" ? "text-white mb-2" : "text-[#0F4C3A] mb-2"} />
          <p className={`text-sm font-medium ${activeReport === "purchase" ? "text-white" : "text-gray-900"}`}>
            Purchase Report
          </p>
          <p className={`text-xs mt-1 ${activeReport === "purchase" ? "text-green-200" : "text-[#8A8678]"}`}>
            Orders received, supplier spend
          </p>
        </button>
      </div>

      {isLoading && <p className="text-sm text-gray-400">Generating report...</p>}
        ...
        <div className="flex flex-col gap-4">
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            <div className="bg-white border border-[#E8E4D9] rounded-lg p-4">
              ...
              <p className="text-2xl font-medium text-[#0F4C3A]">₹{salesData.totalRevenue.toFixed(2)}</p>
            </div>
            <div className="bg-white border border-[#E8E4D9] rounded-lg p-4">
              <p className="text-xs text-[#8A8678] mb-1">Prescriptions</p>
              <p className="text-2xl font-medium text-gray-900">{salesData.prescriptionCount}</p>
            </div>
            <div className="bg-white border border-[#E8E4D9] rounded-lg p-4">
              <p className="text-xs text-[#8A8678] mb-1">GST Collected</p>
              <p className="text-2xl font-medium text-gray-900">₹{salesData.totalGst.toFixed(2)}</p>
            </div>
          </div>
          ...
          {salesData.topMedicines.length > 0 && (
            <div className="bg-white border border-[#E8E4D9] rounded-lg overflow-hidden">
              <div className="px-4 py-3 border-b border-[#E8E4D9]">
                <h2 className="text-sm font-medium text-gray-900">Top medicines by revenue</h2>
              </div>
              <div className="divide-y divide-[#F1EFE8]">
                {salesData.topMedicines.map((med: any, i: number) => (
                  <div key={med.name} className="flex items-center justify-between px-4 py-3">
                    <div className="flex items-center gap-3">
                      <span className="text-xs text-[#8A8678] w-5">{i + 1}</span>
                      <div>
                        <p className="text-sm text-gray-900">{med.name}</p>
                        <p className="text-xs text-[#8A8678]">{med.quantity} units sold</p>
                      </div>
                    </div>
                    <p className="text-sm font-medium text-gray-900">₹{med.revenue.toFixed(2)}</p>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="flex flex-col gap-4">
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-white border border-[#E8E4D9] rounded-lg p-4">
              ...
              <p className="text-2xl font-medium text-gray-900">₹{purchaseData.totalSpend.toFixed(2)}</p>
            </div>
            <div className="bg-white border border-[#E8E4D9] rounded-lg p-4">
              <p className="text-xs text-[#8A8678] mb-1">Orders Received</p>
              <p className="text-2xl font-medium text-gray-900">{purchaseData.orderCount}</p>
            </div>
          </div>
          ...
          {purchaseData.supplierBreakdown.length > 0 && (
            <div className="bg-white border border-[#E8E4D9] rounded-lg overflow-hidden">
              <div className="px-4 py-3 border-b border-[#E8E4D9]">
                <h2 className="text-sm font-medium text-gray-900">Spend by supplier</h2>
              </div>
              <div className="divide-y divide-[#F1EFE8]">
                {purchaseData.supplierBreakdown.map((s: any) => (
                  <div key={s.name} className="flex items-center justify-between px-4 py-3">
                    <div>
                      ...
                      <p className="text-xs text-[#8A8678]">{s.orders} order{s.orders !== 1 ? "s" : ""}</p>
                    </div>
                    <p className="text-sm font-medium text-gray-900">₹{s.spend.toFixed(2)}</p>
                  </div>
                ))}
              </div>
            </div>
