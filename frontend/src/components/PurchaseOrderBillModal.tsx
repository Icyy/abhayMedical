...
const PurchaseOrderBillModal = ({ medicine, supplier, estimatedQuantity, onClose, onOrderCreated }: PurchaseOrderBillModalProps) => {
  ...
  const margin = sellingPrice > 0 && pricePerUnit > 0
    ...
    <div className="fixed inset-0 bg-black/50 flex items-end sm:items-center justify-center z-50 p-0 sm:p-4">
      <div className="bg-white rounded-t-2xl sm:rounded-lg w-full sm:max-w-lg flex flex-col max-h-[95vh]">
        ...
        <div className="flex justify-between items-start p-5 border-b border-[#F1EFE8] flex-shrink-0">
          <div>
            <h2 className="text-base font-medium text-gray-900">New Purchase Bill</h2>
        ...
        <div className="flex-1 overflow-y-auto p-5">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="flex flex-col gap-1 sm:col-span-2">
              <label className="text-xs text-[#8A8678]">Batch number <span className="text-red-400">*</span></label>
              <input
            ...
            <div className="flex flex-col gap-1">
              <label className="text-xs text-[#8A8678]">Quantity ordered <span className="text-red-400">*</span></label>
              <input
                type="number"
                min={1}
            ...
            <div className="flex flex-col gap-1">
              <label className="text-xs text-[#8A8678]">
                Purchase price/unit ₹
                <span className="text-[#8A8678] ml-1">({supplier.discountPercent}% off MRP)</span>
              </label>
              <input
                type="number"
                min={0}
                step="0.01"
                {...register("pricePerUnit", { valueAsNumber: true, required: true })}
                className="border border-gray-200 rounded-md px-3 py-2 text-sm focus:outline-none focus:border-green-400"
              />
            </div>

            <div className="flex flex-col gap-1">
              <label className="text-xs text-[#8A8678]">Selling price/unit (MRP) ₹</label>
              <input
                type="number"
                min={0}
                step="0.01"
          ...
          <div className="mt-4 bg-[#F7F5F0] rounded-lg p-3 text-xs">
            ...
            <div className="flex justify-between mb-1 text-[#8A8678]">
              ...
              <span className="text-gray-900">{quantity} × ₹{pricePerUnit.toFixed(2)}</span>
            </div>
            <div className="flex justify-between mb-1 text-[#8A8678]">
        ...
        <div className="p-5 border-t border-[#F1EFE8] flex-shrink-0">
          ...
          >
            {isSubmitting ? "Creating order..." : `Create Purchase Order · ₹${totalCost.toFixed(2)}`}
          </button>
        </div>
      </div>
    </div>
