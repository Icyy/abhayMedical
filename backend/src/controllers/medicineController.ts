  ...
  try {
    ...
    const enriched = medicines.map(med => {
      ...
      return {
        ...
        expiringBatches: med.batches.filter(b => new Date(b.expiryDate) < in30Days && b.stockUnits > 0).length,
      }
    })

    // Apply status filter after computing
    const filtered = status ? enriched.filter(m => m.status === status) : enriched
...
export const addMedicine = async (req: AuthRequest, res: Response) => {
  try {
    const {
      name, unit, packType, unitsPerPack, category, gstPercent, mrp,
      // batch details for first delivery
      batchNumber, manufacturingDate, expiryDate, purchasePrice, stockUnits, supplierId
    } = req.body
...
export const addBatch = async (req: AuthRequest, res: Response) => {
  try {
    const { id } = req.params
    const { batchNumber, manufacturingDate, expiryDate, purchasePrice, stockUnits, supplierId } = req.body
